...
🔗 프로젝트 종료 후 개인 배포: [배포 저장소](https://github.com/battlegroundcallofduty/Moong_deploy) | [라이브 데모](https://moong.site)
...
* **구성**: 5인 팀 프로젝트
* [원본 팀 레포](https://github.com/solrimna/moong)
...
- **결과**: 임시저장한 글을 다시 불러와도 해시태그가 정상적으로 표시되며, 저장된 태그 데이터가 유실되지 않음을 확인
...
- **결과**: 해당 특수 구역을 포함한 테스트 케이스에서 중복 태그 없이 의도한 지역 태그만 생성되는 것을 확인
...
- **결과**: 지역명과 무관한 키워드는 더 이상 지역 태그로 오분류되지 않고, 일반 키워드 태그로 정상 분류됨
...
### ~~배포 계획~~  →  AWS EC2로 개인 배포 완료 ([Live Demo](https://moong.site))
| 구분 | 내용 |
|------|------|
| 무료 배포일 경우 | Railway |
| 유료 배포일 경우 | AWS EC2 |
